...
OpenSSLAdapter::ContinueSSL() {
  ...
  case SSL_ERROR_NONE:
    if (!SSLPostConnectionCheck(ssl_, ssl_host_name_.c_str())) {
      ...
      printf("continue ssl Check Failed!\n");
  ...
  case SSL_ERROR_WANT_READ:
  	printf("continue ssl error code:SSL_ERROR_WANT_READ!\n");
  ...
  case SSL_ERROR_WANT_WRITE:
  	printf("continue ssl error code:SSL_ERROR_WANT_WRITE!\n");
    break;

  case SSL_ERROR_ZERO_RETURN:
  default:
  	printf("continue ssl error code:SSL_ERROR_ZERO_RETURN!\n");
...
OpenSSLAdapter::Send(const void* pv, size_t cb) {
  ...
  printf("OpenSSLAdapter::Send:%d, %d\n", cb, state_);
  ...
  printf("OpenSSLAdapter::start\n");
  ssl_write_needs_read_ = false;

  int code = SSL_write(ssl_, pv, checked_cast<int>(cb));
  switch (SSL_get_error(ssl_, code)) {
  case SSL_ERROR_NONE:
  	printf("OpenSSLAdapter::success\n");
    //LOG(LS_INFO) << " -- success";
    return code;
  case SSL_ERROR_WANT_READ:
  	printf("OpenSSLAdapter::error want read\n");
    //LOG(LS_INFO) << " -- error want read";
    ssl_write_needs_read_ = true;
    SetError(EWOULDBLOCK);
    break;
  case SSL_ERROR_WANT_WRITE:
  	printf("OpenSSLAdapter:: error want write\n");
    //LOG(LS_INFO) << " -- error want write";
    SetError(EWOULDBLOCK);
    break;
  case SSL_ERROR_ZERO_RETURN:
  	printf("OpenSSLAdapter:: remote side closed\n");
  ...
  default:
    ...
    printf("OpenSSLAdapter::  error code:%d\n", code);
...
bool OpenSSLAdapter::SSLPostConnectionCheck(SSL* ssl, const char* host) {
  bool ok = true;//VerifyServerName(ssl, host, ignore_bad_cert());
