...
public class HinestPresence
	...
	private static final Logger     log = Logger.getLogger(HinestPresence.class.getName());
	...
	private static final String     ID                      = "hinestpresence";
//	// special prefix to mark user
//	private static final String PRE_USER = "usr_";
//	// special prefix to mark dev
//	private static final String PRE_DEV = "dev_";
		...
		if ( extPresenceProcessorsClasses != null ){
			for ( String clazz : extPresenceProcessorsClasses ) {
				try {
					...
					Logger.getLogger( HinestPresence.class.getName() ).log( Level.SEVERE, null, ex );
				}
			}
		}
	}
	...
	if (packet.getElemName() == Iq.ELEM_NAME) {
		...
		synchronized (session) {
			try {
				...
				switch (pres_type) {
				case out_initial :
					HinestLog.debug("out_initial~~~");
					processOutInitial(packet, session, results, settings, pres_type);

					break;

				case out_subscribe :
					HinestLog.debug("out_subscribe~~~");
					
					//linlinno
					//just dev and usr can bind each other
					
					if (((packet.getStanzaFrom().getDomain().equals(DEV_DOMAIN) && packet.getStanzaTo().getDomain().equals(USR_DOMAIN)))
							|| ((packet.getStanzaFrom().getDomain().equals(USR_DOMAIN) && packet.getStanzaTo().getDomain().equals(DEV_DOMAIN)))) {				
//		    		if (((packet.getStanzaFrom().getLocalpart().startsWith(PRE_DEV) && packet.getStanzaTo().getLocalpart().startsWith(PRE_USER)))
//							|| ((packet.getStanzaFrom().getLocalpart().startsWith(PRE_USER) && packet.getStanzaTo().getLocalpart().startsWith(PRE_DEV)))) {
						
						HinestLog.debug("usr bind dev,dev bind usr~");
		    			//processOutSubscribe(packet, session, results, settings, pres_type);
					}
		    		else{
		    			HinestLog.debug("not usr bind dev,not dev bind usr~");
		    			Element presence = null;
			    		Packet  result   = null;

			    		presence = new Element(PRESENCE_ELEMENT_NAME);
			    		presence.setAttribute("type", StanzaType.unsubscribed.toString());
...
//		    		if (((packet.getStanzaFrom().getLocalpart().startsWith(PRE_DEV) && packet.getStanzaTo().getLocalpart().startsWith(PRE_USER)))){
		    			...
		    			if (buddies != null && buddies.length>=1) {
		    				...
		    				HinestLog.debug(session.getBareJID() + " | Buddies found: " + Arrays
	    							.toString(buddies));
		    				...
		    				HinestLog.debug("No buddies found!!!!");
					...
					break;
				case out_unsubscribe :
					HinestLog.debug("out_unsubscribe~~~");
					processOutSubscribe(packet, session, results, settings, pres_type);

					break;

				case out_subscribed :
					HinestLog.debug("out_subscribed~~~");
...
//		    		if (((packet.getStanzaFrom().getLocalpart().startsWith(PRE_DEV) && packet.getStanzaTo().getLocalpart().startsWith(PRE_USER)))){
		    			...
		    			if (buddies != null && buddies.length>=1) {
				    		...
				    		presence.setXMLNS(XMLNS);
				    		presence.setAttribute("to", packet.getStanzaTo().toString());
					...
					break;
				case out_unsubscribed :
					HinestLog.debug("out_unsubscribed~~~");
					processOutSubscribed(packet, session, results, settings, pres_type);

					break;

				case in_initial :
					HinestLog.debug("in_initial~~~");
					processInInitial(packet, session, results, settings, pres_type);

					break;

				case in_subscribe :
					HinestLog.debug("in_subscribe~~~");
...
//							|| ((packet.getStanzaFrom().getLocalpart().startsWith(PRE_USER) && packet.getStanzaTo().getLocalpart().startsWith(PRE_DEV)))) {
				...
				case in_unsubscribe :
					HinestLog.debug("in_unsubscribe~~~");
					processInUnsubscribe(packet, session, results, settings, pres_type);

					break;

				case in_subscribed :
					HinestLog.debug("in_subscribed~~~");
					processInSubscribed(packet, session, results, settings, pres_type,repo);

					break;

				case in_unsubscribed :
					HinestLog.debug("in_unsubscribed~~~");
					processInUnsubscribed(packet, session, results, settings, pres_type);

					break;

				case in_probe :
					HinestLog.debug("in_probe~~~");
...
//		sendPresenceBroadcast( StanzaType.get, session, SUB_TO, results, presence, null, null );
	...
	protected void processInSubscribed(Packet packet, XMPPResourceConnection session,
			Queue<Packet> results, Map<String, Object> settings, PresenceType pres_type,NonAuthUserRepository repo)
					throws NotAuthorizedException, TigaseDBException, NoConnectionIdException {
		SubscriptionType curr_sub = roster_util.getBuddySubscription(session, packet
				.getStanzaFrom());
		...
		if (subscr_changed) {
			...
			//linlinno
			SubscriptionType current_subscription = roster_util.getBuddySubscription(session, packet.getStanzaFrom());
			HinestLog.debug("!current_subscription:"+current_subscription.toString());
			if(current_subscription == SubscriptionType.both){
				String owner = repo.getPublicData(packet.getStanzaTo().getBareJID(), HISNET, "owner", null);
				
				HinestLog.debug("bind from:"+packet.getStanzaFrom().toString());
				HinestLog.debug("bind to:"+packet.getStanzaTo().toString());

				if(packet.getStanzaFrom().getDomain().equals(USR_DOMAIN) && packet.getStanzaTo().getDomain().equals(DEV_DOMAIN)){
					if(null == owner){
						//first bind,save the owner
						HinestLog.debug("first bind,save the owner");
						session.setPublicData(HISNET, "owner", packet.getStanzaFrom().getBareJID().toString());
						
					}else{
						if(!packet.getStanzaFrom().getBareJID().toString().equals(owner)){
							//not the same user bind dev,delete files on cloud
							HinestLog.debug("not the same user bind dev,delete files on cloud!to do...");
							session.setPublicData(HISNET, "owner", packet.getStanzaFrom().getBareJID().toString());
						}else{
							//the same user bind dev,do nothing
							HinestLog.debug("the same user bind dev,do nothing...");
						}
					}
				}
			}
